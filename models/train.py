import sys, os, time
import argparse
import keras_models, dataset_utils
import logging
...
def create_model(backbone, num_classes, num_channels, \
    ...
    if backbone=='deep_loc':
        model = keras_models.deep_loc(num_classes,
                                      k=width_factor,
        ...
        model = keras_models.custom_network(num_classes=num_classes,
...
#(args.dataset_type, args.labels_type, args.do_mask, args.use_rfp, args.use_nuc, args.dropout_rate, args.backbone, args.uniform_sampling, args.save_prefix)
def get_folder_name(args):
    save_name = args.dataset + '_dense'+ str(args.dense1_size) + '_features'+ str(args.num_features) +\
                ...
                '_avg_pool' * args.avg_pool + ('_width' + str(args.width_factor)) * (args.backbone == 'deep_loc' )
